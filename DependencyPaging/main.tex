...
To bound the total cost incurred by our algorithm ($ALG$) over the entire sequence, we decompose its cost into two components: the cost of bypasses (the total budget poured) and the cost of fetches (cache misses). 

\begin{lemma}[Global Bound on Bypass Cost]
\label{lemma:bypass_cost_bound}
Over the entire request sequence, the total cost of bypasses incurred by $ALG$ is less than or equal to the total cost of fetches.
\end{lemma}
\begin{proof}
The total bypass penalty paid by $ALG$ corresponds exactly to the continuous budget poured into the fractional counters $w_q$ throughout the sequence. By the design of the algorithm, a counter maxes out at $1$, at which point it triggers a mark and a corresponding fetch event (if the page is not cached). Although counters reset to $0$ at the start of every phase, the total accumulated budget poured into any page before a reset or a fetch is strictly bounded by $1$. By directly charging the fractional budget poured into these variables to the eventual actual page fetches they induce (or the capacity they consume), we guarantee that the total budget poured across all phases—and thus the total bypass penalty—is strictly bounded by the total cost of fetches across all phases.
\end{proof}
...
Because $b'_t \le b_t$, we can injectively charge $ALG$'s poured budget directly against $OPT$'s bypass penalty. We define $OPT$'s \textit{apportioned bypass cost} for the time window of phases $i-1$ and $i$ as exactly the amount of budget $ALG$ pours during these two phases for requests in $\mathcal{B}_{OPT}$. Summing this apportioned cost over all disjoint phase pairs naturally bounds $OPT$'s true total bypass cost without double-counting.

Consider a page $q \in X$. Because $q \in M$, $ALG$ marked $q$ during phase $i-1$ or $i$. This means $ALG$ poured exactly $1$ total unit of fractional credit into $q$'s counter strictly within this time window. Every time $ALG$ pours budget into $q$ while processing some request $t$, it implies $q$ is a required dependency for $p_t$. Since $q \in X$, $OPT$ does \textit{not} have $q$ in its cache at any point during these phases. Consequently, $OPT$ is physically incapable of satisfying request $t$ and \textit{must} bypass it ($t \in \mathcal{B}_{OPT}$).
