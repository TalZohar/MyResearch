...
A marking algorithm processes a request sequence in a series of disjoint phases. At the beginning of a phase, all pages are considered ``unmarked.'' As requests arrive, the requested pages (and their dependencies) are ``marked.'' The fundamental rule of a marking algorithm is that it \textbf{never evicts a marked page}. When the cache is full and an eviction is required, the algorithm is forced to choose an unmarked page. A phase naturally terminates when an eviction is needed but the cache is entirely full of marked pages.

Let $m$ denote the number of new pages requested in a phase (pages that were not in the cache at the start of the phase). 
...
To prove the $O(l)$-competitiveness, we bound the number of misses on ``old pages'' (pages that were in the cache at the start of the phase). 
...
\begin{theorem}
\label{thm:deterministic_O_l}
The generic deterministic marking algorithm is $O(l)$-competitive.
\end{theorem}
\begin{proof}
We are interested in the total number of misses on old pages. An old page from chain $C_j$ incurs a miss if and only if it is evicted and subsequently refetched. 

By Lemma \ref{lemma:chain_marking}, the very first time an evicted old page from $C_j$ is refetched, $C_j$ becomes a marked chain. 
A marked chain never becomes unmarked again for the remainder of the phase. 
...
Summing this bound over all $l$ chains, the total number of misses on old pages is at most $m \cdot l$. The algorithm also pays exactly $m$ misses to fetch the new pages. Thus, the total cost for the phase is $m + m \cdot l = m(l + 1)$. By Lemma \ref{lemma:opt_bound}, $OPT$ pays an amortized cost of at least $m/2$ per phase, meaning the algorithm is $O(l)$-competitive.
\end{proof}
...
\end{corollary}
\begin{proof}
By Theorem \ref{thm:deterministic_O_l}, the algorithm is $O(l)$-competitive. Since standard LRU is a valid implementation of this generic marking algorithm and LRU is intrinsically $O(k)$-competitive, the algorithm achieves an overall competitive ratio of $O(\min(k,l))$.
\end{proof}

\section{A Randomized $O(\log l)$-Competitive Algorithm}
The paper \cite{dallot2024dependency} provides an expected $O(\min(\log k, \log l))$-competitive randomized algorithm. Here, we provide an alternative, simpler analysis to show expected $O(\log l)$-competitiveness. \textit{(TODO: Show that the algorithm is also expected $O(\log k)$-competitive.)}
...
By Claim \ref{claim:coin_removal}, we only remove coins from marked chains. This means there may be coins residing on chains that are already marked, but this is only in our favor, as it leaves fewer coins distributed among the unmarked chains. 

By Claim \ref{claim:total_coins}, the total number of coins in the entire system at any given moment is bounded by $m$. Since the algorithm distributes new coins uniformly at random among all currently unmarked chains, by symmetry, the expected number of coins residing on any specific unmarked chain is bounded by the maximum total coins in the system divided by the number of unmarked chains, $l - j + 1$. Therefore:
$$E[t_j] \le \frac{m}{l - j + 1}$$
\end{proof}

\begin{theorem}
\label{thm:randomized_O_log_l}
The randomized dependency-aware marking algorithm is $O(\log l)$-competitive.
\end{theorem}
\begin{proof}
...
By Claim \ref{claim:coin_removal}, coins are exclusively discarded from a chain once it becomes marked. Because marked chains do not receive new coins (as evictions only target unmarked chains), the maximum number of coins that can be discarded from chain $C_j$ is bounded by $t_j$, the number of coins it possessed at the exact moment it became marked. Summing these expectations over all $l$ chains provides an upper bound on the total expected cost for old pages:
\begin{align*}
    E[\text{misses on old pages}] &\le \sum_{j=1}^{l} E[t_j] \\
    &\le \sum_{j=1}^{l} \frac{m}{l - j + 1} \quad \text{(by Claim \ref{claim:expected_marked})} \\
    &= m \sum_{i=1}^{l} \frac{1}{i} \\
    &= O(m \log l)
\end{align*}

This bounds the expected cost of fetching old pages. Furthermore, the algorithm incurs exactly $m$ misses to fetch the $m$ new pages requested during the phase. Thus, the total expected cost of the algorithm for the phase is $m + O(m \log l) = O(m \log l)$. 

By Lemma \ref{lemma:opt_bound}, $OPT$ incurs an amortized cost of at least $m/2$ misses per phase. Therefore, our randomized algorithm yields an overall expected competitive ratio of $O(\log l)$.
\end{proof}
