...
\textbf{The Inadequacy of the Global Constraint:} To see why the global capacity constraint alone is insufficient, consider the following counterexample. Suppose we have a cache of capacity $k$. There is a set $B$ of $k-1$ "bottom" pages, and a set $T$ of $k-1$ "top" pages. The dependencies are configured such that \textit{every} top page $t \in T$ depends on \textit{all} $k-1$ bottom pages in $B$.
...
Now, suppose we formulate a simplified LP using \textit{only} the global capacity constraint ($\sum_{p \in \mathcal{P}} x_p \ge (2k-2) - k = k-2$) and the dependency constraints ($x_t \ge x_b$ for all $t \in T, b \in B$). 
...
\textbf{How Our Subset Constraints Fix the Issue:} Our LP prevents this by enforcing $\sum_{p \in S} x_p \ge q(S)$ for \textit{every} infeasible subset. 

Consider the subset $S = T$ (consisting solely of the $k-1$ top pages). What is $q(T)$? To make any subset $T' \subseteq T$ feasible, its closure must be at most $k$. Because the closure of any top page immediately brings all $k-1$ bottom pages into the cache, we can retain at most $1$ top page. To make $S = T$ feasible, we are forced to exclude $q(T) = |T| - 1 = k - 2$ pages.

This generates the specific LP constraint:
$$ \sum_{t \in T} x_t \ge k - 2 $$
If we evaluate the cheating fractional solution ($x_t = 0.5$), the sum is $\frac{k-1}{2}$. For any $k > 3$ (e.g., $k=5$), the sum evaluates to $2$, which strictly violates the required bound of $k-2 = 3$. 
